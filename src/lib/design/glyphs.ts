...
export const POSES: Record<string, PoseFn> = {
	...
	facepull: (d) => {
		const L = (a: number, b: number) => a + (b - a) * d;
		const hx = L(0.4, 0.1), hy = L(0.82, 0.9);
		const ex = L(0.22, -0.12), ey = L(0.8, 0.86);
		return { segs: [
			[0.55, 0.02, 0.55, 1.04, 0.018],                                 // cable tower
			[-0.14, 0.03, 0.07, 0.03, 0.022],                                // staggered stance
			[-0.05, 0.03, 0.01, 0.28, 0.034], [0.01, 0.28, -0.01, 0.53, 0.045],
			[-0.01, 0.53, -0.04, 0.82, 0.058],                               // slight lean back
			[-0.02, 0.8, ex, ey, 0.024], [ex, ey, hx, hy, 0.02],
			[hx, hy, 0.5, 0.87, 0.012]                                        // rope to the pulley
		], circs: [[-0.05, 0.94, 0.056], [0.55, 0.87, 0.03]], box: [-0.28, 0.64, 1.1] };
	},
	rdl: (d) => {
		const L = (a: number, b: number) => a + (b - a) * d;
	...
	calf: (d) => {
		// on a step: toes on the block, heel swings from below the edge to high tiptoe
		const rise = d * 0.1, heelY = 0.1 + d * 0.16;
		return { segs: [
			[0.04, 0.05, 0.3, 0.05, 0.05],                                  // step block
			[0.1, 0.12, -0.08, heelY, 0.022],                               // foot pivoting on toe
			[-0.05, heelY + 0.02, -0.02, 0.36 + rise, 0.034],
			[-0.02, 0.36 + rise, 0, 0.62 + rise, 0.046],
			[0, 0.62 + rise, 0, 0.9 + rise, 0.056],                          // trunk
			[-0.16, 0.9 + rise, 0.13, 0.9 + rise, 0.03],                     // machine shoulder pad
			[-0.16, 1.24, -0.16, 0.92 + rise, 0.02]                          // pad arm from above
		], circs: [[0.03, 1.03 + rise, 0.055]], box: [-0.3, 0.38, 1.26] };
	},
	plankll: (d) => {
	...
	cph: (d) => {
		// body rigid in line, top knee on the bench; ONLY the bottom leg lifts to meet it
		const L = (a: number, b: number) => a + (b - a) * d;
		const bk = [0.3, L(0.16, 0.26)], bf = [L(0.38, 0.55), L(0.04, 0.24)];
		return { segs: [
			[0.62, 0.24, 0.9, 0.24, 0.055], [0.66, 0.02, 0.66, 0.16, 0.016], [0.86, 0.02, 0.86, 0.16, 0.016],
			[0.7, 0.31, 0.36, 0.315, 0.026],                                  // top shin, knee on bench
			[0.36, 0.315, 0.04, 0.32, 0.042],                                 // top thigh
			[0.04, 0.32, -0.38, 0.32, 0.056],                                 // trunk, held in line
			[0.04, 0.3, bk[0], bk[1], 0.036], [bk[0], bk[1], bf[0], bf[1], 0.026], // bottom leg lifting
			[-0.38, 0.32, -0.44, 0.03, 0.024], [-0.44, 0.03, -0.26, 0.03, 0.02]
		], circs: [[-0.51, 0.36, 0.052]], box: [-0.6, 0.94, 0.6] };
	},
	deadbug: (d) => {
		const L = (a: number, b: number) => a + (b - a) * d;
		const a = L(1.57, 2.75);
