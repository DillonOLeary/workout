<script lang="ts">
	...
	/**
	 * Seven cells, one vocabulary: volt = lifted, ink = ran, white with an ink
	 * outline = stretched, dashed = today and only today — never a state.
	 */
...
<div class="strip" role="list" aria-label="This week">
	{#each cells as c (c.key)}
		...
		>
			...
			<span class="box">{#if (c.lifted || c.stretched) && c.ran}<span class="runbar"></span>{/if}</span>
		</div>
	{/each}
</div>
...
<style>
	...
	.cell.stretched .box { background: var(--white); border-color: var(--ink); }
	.cell.lifted .box { background: var(--volt); border-color: var(--ink); }
	.cell.ran:not(.lifted):not(.stretched) .box { background: var(--ink); border-color: var(--ink); }
