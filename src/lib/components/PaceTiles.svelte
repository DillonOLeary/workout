<script lang="ts">
	...
	/**
	 * The running average, as three numbers: lifts a week, run minutes a week,
	 * days you ran a week — each over the trailing four weeks, each with where
	 * it came from underneath. The calendar above says what happened; this says
	 * whether it adds up to the habit you wanted, and which way it is moving.
	 *
	 * The run's goal is the plan's own `runTarget`, so the one number with a
	 * goal is measured against it instead of against a feeling. Three tiles
	 * across at every width — they are one comparison, not three cards — which
	...
	const tiles = $derived(
		[
			{ key: 'lifts', caps: 'Lifts', value: rateLabel(pace.lifts.per), subs: [paceLabel(pace.lifts.per, pace.lifts.prev)] },
			runs && {
				key: 'runmin',
				caps: 'Run min',
				value: rateLabel(minutes),
				subs: [...(runTarget ? [`goal ${runTarget}`] : []), paceLabel(minutes, Math.round(pace.runMinutes.prev))]
			},
			runs && { key: 'rundays', caps: 'Run days', value: rateLabel(pace.runDays.per), subs: [paceLabel(pace.runDays.per, pace.runDays.prev)] }
		].filter((t) => !!t)
	);
</script>
...
<div class="tiles">
	{#each tiles as t (t.key)}
		<div class="tile">
			...
			{#each t.subs as sub (sub)}<span class="sub">{sub}</span>{/each}
		</div>
	{/each}
</div>
